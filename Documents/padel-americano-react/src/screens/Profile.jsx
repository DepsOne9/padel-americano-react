    ...
    <div className="card" style={{ marginBottom: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => setOpen(o => !o)}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: 14, letterSpacing: '-0.3px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{th.name || 'Tournament'}</div>
          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>{fmtDate(th.date)} · {th.ppg || 24}pts/game</div>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontSize: 26, fontWeight: 800, color: 'var(--accent)', lineHeight: 1, letterSpacing: '-1px' }}>{th.points}</div>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
        {[
          ...
          <div key={cell.l} style={{ flex: 1, background: 'var(--surface)', borderRadius: 10, padding: '7px 6px', textAlign: 'center' }}>
            <div style={{ fontSize: 16, fontWeight: 800, color: cell.c, letterSpacing: '-0.5px' }}>{cell.v}</div>
            <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--muted)' }}>{cell.l}</div>
          </div>
        ))}
      </div>
        ...
        <div style={{ marginTop: 12, borderTop: '1px solid var(--border)', paddingTop: 10 }}>
          ...
          {isMe && (
            <button
              ...
              style={{ width: '100%', marginTop: 10, padding: 9, background: 'rgba(255,92,92,0.07)', border: '1px solid rgba(255,92,92,0.2)', borderRadius: 10, color: 'var(--accent2)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
            >
...
function UsernameSetup({ googleUser, onDone }) {
  ...
  const finish = async () => {
    ...
    try {
      ...
      onDone(profile)
      toast(`Welcome, ${uname}! 🎾`)
    } catch (e) { toast('Setup failed — try again', true) }
    setSaving(false)
  }
  ...
  return (
    <div style={{ padding: '24px 16px 90px' }}>
      <div style={{ textAlign: 'center', marginBottom: 20 }}>
        {googleUser.photoURL
          ? <img src={googleUser.photoURL} style={{ width: 64, height: 64, borderRadius: '50%', border: '2px solid var(--accent)', display: 'block', margin: '0 auto 10px' }} alt="" />
          : <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(0,229,160,0.08)', border: '2px solid var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, margin: '0 auto 10px' }}>👤</div>
        }
        <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: '-0.3px' }}>{googleUser.displayName || 'Google User'}</div>
        <div style={{ fontSize: 12, color: 'var(--muted)' }}>{googleUser.email}</div>
      </div>

      <div style={{ background: 'rgba(0,229,160,0.05)', border: '1px solid rgba(0,229,160,0.15)', borderRadius: 14, padding: '14px 16px', marginBottom: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--accent)', marginBottom: 4 }}>One more step — choose your username</div>
        <div style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1.5 }}>This is your public name in the app. It links your name in tournament player lists to this account.</div>
      </div>

      <div className="fg">
        <label>Username</label>
        <input type="text" value={username} autoCapitalize="none" autoCorrect="off" spellCheck="false"
          onChange={e => setUsername(e.target.value.toLowerCase())}
          style={{ fontSize: 18, fontWeight: 800, letterSpacing: 1 }}
        />
        <div style={{ marginTop: 8, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '10px 12px' }}>
          {[
            { ok: charsOk, text: 'Only letters (a–z), numbers, and underscores _' },
            { ok: lenOk,   text: 'Between 3 and 30 characters' },
          ].map((r, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 12, color: r.ok ? 'var(--accent)' : 'var(--muted)', marginBottom: i === 0 ? 5 : 0 }}>
      ...
      <div className="fg">
        ...
        <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 5, lineHeight: 1.5 }}>
          Enter this <strong style={{ color: 'var(--text)' }}>exactly</strong> as you type your name in tournament player lists.
        </div>
      </div>
    ...
    <div style={{ paddingBottom: 90 }}>
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        {isMe && player.legacy_points > 0 && (
          <div style={{ background: 'rgba(255,209,102,0.07)', border: '1px solid rgba(255,209,102,0.2)', borderRadius: 12, padding: '10px 14px', fontSize: 12, color: 'var(--accent3)', marginTop: 8 }}>
...
function StatsSection({ uid }) {
  ...
  useEffect(() => { fbGet(`players/${uid}`).then(p => setPlayer(p)) }, [uid])
  ...
  const handleRemove = async (code) => {
    ...
    await fbSet(`players/${uid}`, { ...player, tournaments: updated })
    setPlayer({ ...player, tournaments: updated })
    if (currentUser?.uid === uid) setCurrentUser(prev => ({ ...prev, tournaments: updated }))
    toast('Tournament removed ✓')
  }
...
function MyAccount({ u, onSignOut }) {
  ...
  const doUsernameChange = async () => {
    ...
    setSavingUser(false); setEditingUsername(false)
  ...
  const doDisplayNameChange = async () => {
    ...
    setSavingDisplay(false); setEditingDisplay(false)
    ...
    <div style={{ paddingBottom: 90 }}>
      <div style={{ textAlign: 'center', padding: '20px 16px 16px' }}>
        <div style={{ position: 'relative', width: 82, margin: '0 auto 12px' }}>
          <div style={{ width: 82, height: 82, borderRadius: '50%', background: 'rgba(0,229,160,0.08)', border: '2px solid var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, fontWeight: 800, color: 'var(--accent)', overflow: 'hidden' }}>
            {avatarContent}
          </div>
        ...
        {!editingUsername ? (
          <div>
            <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-1px' }}>{u.username}</div>
          ...
          <div style={{ marginTop: 10 }}>
            <input value={newUsername} onChange={e => setNewUsername(e.target.value.toLowerCase())} autoFocus autoCapitalize="none"
              style={{ width: '100%', background: 'var(--bg)', border: '1px solid var(--accent)', borderRadius: 12, padding: '10px 14px', color: 'var(--text)', fontSize: 15, fontWeight: 700, outline: 'none', textAlign: 'center', marginBottom: 8 }}
            />
      ...
      <div style={{ padding: '0 16px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(0,229,160,0.05)', border: '1px solid rgba(0,229,160,0.15)', borderRadius: 12, padding: '9px 12px' }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 700, letterSpacing: '-0.3px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{u.displayName || u.username}</div>
          </div>
        ...
        {editingDisplay && (
          <div style={{ marginTop: 8, background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: '10px 12px' }}>
            <input type="text" placeholder="e.g. Carlos" value={newDisplay} onChange={e => setNewDisplay(e.target.value)}
              style={{ width: '100%', background: 'var(--bg)', border: '1px solid var(--accent)', borderRadius: 10, padding: '9px 12px', color: 'var(--text)', fontSize: 14, fontWeight: 600, outline: 'none', marginBottom: 8 }}
            />
    ...
    return (
      <div className="screen">
        ...
        <div style={{ padding: '24px 16px 0' }}>
          <div style={{ background: 'rgba(255,209,102,0.07)', border: '1px solid rgba(255,209,102,0.2)', borderRadius: 14, padding: '14px 16px', marginBottom: 20 }}>
          ...
          <div style={{ textAlign: 'center', fontSize: 12, color: 'var(--muted)', marginTop: 14, lineHeight: 1.6 }}>Your Google account is only used for sign-in.<br />Choose any username after signing in.</div>
        </div>
      </div>
    )
  }
